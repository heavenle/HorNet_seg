...
from osgeo import ogr, gdalconst, osr, gdal
import glob
import models
import decode_heads
import copy
...
def main():
    ...
    parser.add_argument('--save_path',
                        default=r'./output_bigdata512__20221109_build_large',
                        help='Image file')
    parser.add_argument('--config',
                        default=r'./bigdata512_build_20221109_no_use_guowangtong/upernet_hornet_large_gf_512_160k_bigdata_build.py',
                        help='Config file')
    parser.add_argument('--checkpoint',
                        default=r'./bigdata512_build_20221109_no_use_guowangtong/iter_160000.pth',
...
def predict_big_test_img(model, img_path, palette, eval_m, save_path, need_gd, slide_window_size=512, overlap_rate=0.25):
    ...
    for x_start in x_idx:
        for y_start in y_idx: 
            ...
            if need_gd:
                ...
                temp_img = copy.deepcopy(croped_img[:, :, 2])
                croped_img[:, :, 2] = croped_img[:, :, 0]
                croped_img[:, :, 0] = temp_img
    ...
    if eval_m:
        l_dataset = gdal.Open(os.path.join(os.path.dirname(os.path.dirname(img_path)), "data_labels", os.path.basename(img_path)))
        label = l_dataset.ReadAsArray(0, 0, l_dataset.RasterXSize, l_dataset.RasterYSize).astype(np.uint8)
        ret_metrics = eval_metrics(
            ...
            metrics='mIoU',
            nan_to_num=-1)
        print("Img[{}]==>Iou[{}]==>mIou[{}]==>all_acc[{}]".format(os.path.basename(img_path), ret_metrics['IoU'],
                                                        ret_metrics['IoU'].sum() / num_classes,
                                                        ret_metrics['Acc'].sum() / num_classes))
        with open(os.path.join(save_path, "img_iou.txt"), "a") as F:
            F.write("Img[{}]==>Iou[{}]==>mIou[{}]==>all_acc[{}]".format(os.path.basename(img_path), ret_metrics['IoU'],
                                                        ret_metrics['IoU'].sum() / num_classes,
                                                        ret_metrics['Acc'].sum() / num_classes))
